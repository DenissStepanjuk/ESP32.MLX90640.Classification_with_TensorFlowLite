...
void loop() {
  ...
  if (currentMillis - previousMillis >= frameInterval) {
    ...
    // TensorFlowLite_ESP32---------------------------------------------------------------------------------------------------------
      ...
      for (int i = 0; i < 768; i++) {
        input_data[i] = (int8_t)(buffer_SCALE[i]* 127.5);
      }
      ...
      // Передадим предсказание на вебстраничку, а также выведем её в последовательный порт.
      sendJson(jsonString, doc_tx, "new_prediction", prediction);
